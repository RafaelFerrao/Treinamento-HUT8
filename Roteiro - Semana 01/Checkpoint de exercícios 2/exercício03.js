// Elabore um algoritmo que calcule o que deve ser pago por um produto, considerando o preço normal 
// de etiqueta e a escolha da condição de pagamento. Utilize os códigos da tabela a seguir para ler 
// qual a condição de pagamento escolhida e efetuar o cálculo adequado, imprimindo o valor final no 
// console. Código Condição de pagamento :
// 1 - À vista em dinheiro ou cheque, recebe 10% de desconto
// 2 - À vista no cartão de crédito, recebe 15% de desconto
// 3 - Em duas vezes, preço normal de etiqueta sem juros
// 4 - Em três vezes, preço normal de etiqueta mais juros de 10%

let condPagamento = 2
let preco = 100
let valorFinal = 0

switch(condPagamento){
    case 1:
        valorFinal = preco * 0.9
        break;
    case 2:
        valorFinal = preco * 0.85
        break;
    case 3: 
        valorFinal = preco
        break;
    case 4:
        valorFinal = preco * 1.1
        break;
}

console.log('O valor final foi de R$ ' + valorFinal.toFixed(2))